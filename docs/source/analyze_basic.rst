...
# CastroSimulation Analysis Module

## Overview
`CastroSimulation` is a Python post-processing class built on top of **yt** and **NumPy**. It streamlines the extraction, analysis, and visualization of hydrodynamic and plasma properties from [Castro](https://amrex-astro.github.io/Castro/) simulation outputs. 
...
* **Derived Plasma Quantities:** Computes derived field properties such as electron temperature ($T_e$) and heavy-particle temperature ($T_h$) dynamically.
...
### Database Setup (`database_species.json`)
The analysis class requires a JSON file mapping species details (e.g., mass in grams, ionization potential in eV). Ensure `database_species.json` is located in the same directory as the module script:

```json
{
  "H0": { "mass": 1.673e-24, "ion_energy": 13.6 },
  "H1": { "mass": 1.672e-24, "ion_energy": 0.0 },
  "He0": { "mass": 6.646e-24, "ion_energy": 24.6 }
}
```
...
```python
# Extract electron temperature at t = 1.0e-6 s at AMR level 1
field_data = sim.get_field(t=1.0e-6, quantity="T_e", level=1)
...
slice_data = sim.get_field(t=1.0e-6, quantity="density", level=2, positions={'z': 0.5})
```
...
### Plotting Fields with AMR Grids
