...
def test_1d_desy_benchmark():
    ...
    # The runtime options are the parameters that are temporary overwritten in the input file to lauch the simulation.
    # This avoid to modify each time we want to run with differents parameters
    run_castro_simulation(model='gamma_law_2T', runtime_options="castro.add_ext_src=1 castro.cfl = 0.5 amr.n_cell = 32 amr.max_level = 2 castro.diffuse_temp=0 amr.plot_int = 1000000 amr.plot_per = 1e-9 problem.initial_conditions_file=1d_desy_benchmark.h5")
    # Physical tests
